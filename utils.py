...
def create_kd_gauge(kills, deaths):
    """Create a simple KD ratio visualization with just numbers and text"""
    kd_ratio = kills / max(1, deaths)
    kd_color = get_kd_color(kd_ratio)
    
    # Criar uma figura vazia
    fig = go.Figure()
    
    # Adicionar um texto grande centralizado com o valor KD
    fig.add_annotation(
        x=0.5, y=0.65,
        text=f"<b style='font-size:42px; color:{kd_color};'>{kd_ratio:.2f}</b>",
        showarrow=False,
        xref="paper",
        yref="paper"
    )
    
    # Adicionar título do KD ratio
    fig.add_annotation(
        x=0.5, y=0.9,
        text="<b style='font-size:20px; color:#F5B841;'>K/D Performance</b>",
        showarrow=False,
        xref="paper",
        yref="paper"
    )
    
    # Adicionar detalhes de Kills e Deaths
    fig.add_annotation(
        x=0.5, y=0.35,
        text=f"<span style='font-size:16px;'><span style='color:#4CAF50;'>{kills} Kills</span> | <span style='color:#FF5252;'>{deaths} Deaths</span></span>",
        showarrow=False,
        xref="paper",
        yref="paper"
    )
    ...
    # Avaliação do KD
    assessment = "Excelente" if kd_ratio >= 3 else "Bom" if kd_ratio >= 1 else "Precisa melhorar"
    assessment_color = get_kd_color(kd_ratio)
    
    fig.add_annotation(
        x=0.5, y=0.15,
        text=f"<span style='font-size:14px; color:{assessment_color};'>{assessment}</span>",
        showarrow=False,
        xref="paper",
        yref="paper"
    )
...
def display_battle_card(battle, guild_name):
    ...
    try:
        if 'details' in battle and 'guilds' in battle['details']:
            ...
            # Primeiro passo: identificar a guild principal e coletar os dados
            for guild, guild_stats in battle['details']['guilds'].items():
                if guild_name.lower() in guild.lower():
                    ...
                    # Se não é nossa guild, é uma guild inimiga
                    enemy_guilds.append(guild)
            
            # Pegar o nome da primeira guild inimiga encontrada
            enemy_guild = enemy_guilds[0] if enemy_guilds else "Desconhecido"'players'])
                else:
                    # Coletar informações das guilds inimigas
                    enemy_guilds.append({
                        'name': guild,
                        'kills': guild_stats.get('kills', 0),
                        'deaths': guild_stats.get('deaths', 0),
                        'players': len(guild_stats.get('players', []))
                    })
            
            # Segundo passo: identificar a principal guild inimiga 
            # (prioridade para a com mais jogadores, depois para a com mais kills)
            if enemy_guilds:
                # Ordenar por número de jogadores e depois por kills
                enemy_guilds.sort(key=lambda x: (x['players'], x['kills']), reverse=True)
                enemy_guild = enemy_guilds[0]['name']
