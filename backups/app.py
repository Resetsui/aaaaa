...
from api_data_processor import get_battle_data
import utils
...
def refresh_data():
    try:
        logging.info("Iniciando atualização automática dos dados")
        # Atualizar dados de batalha
        ...
        logging.error(f"Erro na atualização automática: {e}")
...
def main():
    ...
    with st.container():
        st.markdown("""
        <div style="background-color: rgba(245, 184, 65, 0.15); padding: 15px; border-radius: 10px; margin-bottom: 20px; border: 1px solid rgba(245, 184, 65, 0.3); box-shadow: 0 2px 8px rgba(0, 0, 0, 0.2);">
            <h4 style="color: #F5B841; margin-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                <span style="font-size: 20px;">⚙️</span> Filtros de Batalha
            </h4>
        </div>
        """, unsafe_allow_html=True)
    ...
    # Abas principais
    tabs = st.tabs(["📊 Visão Geral", "🏆 Ranking de Jogadores", "⚔️ Detalhes de Batalhas", "📈 Comparativos"])
