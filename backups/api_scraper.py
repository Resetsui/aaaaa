...
BATTLES_URL = "https://gameinfo.albiononline.com/api/gameinfo/battles"
...
def get_guild_id(guild_name, max_attempts=3, delay=2):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            logging.warning(f"Guild {guild_name} not found in attempt {attempt+1}/{max_attempts}")
            ...
            logging.warning(f"Request timed out in attempt {attempt+1}/{max_attempts}")
            if attempt < max_attempts - 1:
                time.sleep(delay)
    ...
    logging.warning(f"Guild {guild_name} not found after {max_attempts} attempts")
    return None

def get_guild_battles(guild_id, days=7, max_attempts=5, delay=3):
    """
    ...
    for attempt in range(max_attempts):
        try:
            ...
            # Esta URL usa o parâmetro range=week que é mais confiável
            url = f"https://gameinfo.albiononline.com/api/gameinfo/battles?range=week&offset=0&limit=20&sort=totalfame&guildId={guild_id}"
            ...
            logging.info(f"Tentativa {attempt+1}/{max_attempts} de obter dados de batalhas usando a URL direta: {url}")
            response = requests.get(url, headers=headers, timeout=10)
            
            # Verificar se a resposta foi bem-sucedida
            if response.status_code != 200:
                logging.error(f"Erro HTTP {response.status_code} ao buscar batalhas")
                if attempt < max_attempts - 1:
                    time.sleep(delay)
                continue
...
def process_battle_details(battle_data, guild_name):
    ...
    logging.info(f"Processando detalhes da batalha: {battle_data['battle_id']}")
...
def get_battle_by_id(battle_id, max_attempts=3, delay=2):
    ...
    for attempt in range(max_attempts):
        try:
            logging.info(f"Buscando batalha ID {battle_id}, tentativa {attempt+1}/{max_attempts}")
            ...
            if 'id' not in battle_data:
                logging.warning(f"Resposta para batalha ID {battle_id} não possui o formato esperado")
                if attempt < max_attempts - 1:
                    time.sleep(delay)
                continue
                
            battle_id = battle_data.get('id')
            battle_time = datetime.fromisoformat(battle_data.get('startTime').replace('Z', '+00:00'))
            ...
            logging.warning(f"Timeout ao buscar batalha ID {battle_id}, tentativa {attempt+1}/{max_attempts}")
            if attempt < max_attempts - 1:
                time.sleep(delay)
...
def refresh_battle_data(guild_name, days=30):
    ...
    if not guild_id:
        logging.warning(f"Guild ID não encontrado para {guild_name}, tentando usar batalhas conhecidas")
    ...
    if battles_df.empty:
        logging.warning("Não foi possível obter batalhas pela API padrão, tentando com batalhas conhecidas")
        return get_known_battles(guild_name)
...
def get_known_battles(guild_name):
    ...
    detailed_df = pd.DataFrame(detailed_battles) if detailed_battles else pd.DataFrame()
    logging.info(f"Retrieved {len(detailed_df)} battles using known battle IDs")
